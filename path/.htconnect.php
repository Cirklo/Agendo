...
class dbConnection extends PDO{
	...
	public function __construct($createDB = false){
		$this->databaseSettings();
		$this->dsn = $this->engine.":dbname=".$this->database.";host=".$this->host;
		
		if($createDB)
			$this->dsn = $this->engine.":dbname=;host=".$this->host;
		...
		try {
			...
			if($createDB){
			    $this->query('CREATE DATABASE '.$this->database); // create database
			    $this->query('USE '.$this->database); // select newly created database
			}
	...
	public function databaseSettings(){
		...
		$this->database = 'delmeplz';//"dbtest" or "postgres"
		...
		$this->host = "localhost";
		$this->schema = 'delmeplz';// Name of the schema (same as database for mysql)
