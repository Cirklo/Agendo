...
// *******************************************        calendar stuff made here     ***********************************************************
		   ...
		   if ($calendar->getStatus()==0 or $calendar->getStatus()==2) { //inactive or invisible
			...
			if (isset($_GET['entry'])){
				...
				$sql ="SELECT xfields_name, xfieldsval_value, xfields_label, xfields_type, xfields_id from xfields, resxfields, xfieldsval, xfieldsinputtype where xfieldsval_field=xfields_id and xfields_id=resxfields_field and resxfields_resource=" . $calendar->getResource(). " and xfieldsval_entry=".$calendar->getEntry()." group by xfields_id, xfields_type";
				...
				$entrystart= $arre['dateHour'] + $arre['dateMinutes']/60;
					...
					$nslots=1;
					$user='';
					$sql ="SELECT xfields_name, xfields_label, xfields_type, xfields_id from xfields, resxfields, xfieldsinputtype where xfields_id=resxfields_field and resxfields_resource=". $calendar->getResource()." group by xfields_id, xfields_type";
			}
			...
			echo "<div id=entrydiv class=entrydiv>";
				...
				echo "<table id=entryinner class=entryinner align=center>";
					...
					echo "<tr>";
						...
						echo "<input lang=send type=checkbox onkeypress='return noenter()' name=assistance value=1>";
						echo "</td>";
						...
						echo "<tr>";
							if($arrxfields['xfields_type'] == 1){
								...
								if($formerInputType != $arrxfields['xfields_type']){
									...
									echo "<tr><td colspan=2><table id='".$arrxfields['xfields_name']."'></table></td></tr>";
								}
